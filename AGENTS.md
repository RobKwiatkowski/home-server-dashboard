...
The goal of this repo is to keep the dashboard infrastructure reproducible, simple, and safe to modify with AI coding agents.
...
Do not introduce Kubernetes, Terraform, Ansible, or complex orchestration unless explicitly requested.
...
Quiz and Notes are still exposed directly through ports because they are root-mounted web apps and do not work cleanly under path prefixes like /quiz or /notes.

Do not change this to path-based routing unless you also update the applications to support non-root base paths.
...
Homepage uses its own /api/* endpoints. Do not route /api/* in Caddy to another backend, because that breaks Homepage.
...
The Glances service should be accessible from the Homepage container through Docker DNS:
...
Disk and temperature widgets are currently not fully configured. Do not assume they work without testing the Glances API first.
...
home-webserver-db may still expose port 5432 to the LAN. This should be cleaned up later, but not while making unrelated changes.
...
Do not add public exposure, Cloudflare Tunnel, port forwarding, public DNS, or HTTPS automation unless explicitly requested.
...
Start it with docker compose up -d <service>
Verify it works directly
Add a Homepage tile
Only then consider Caddy routing
Do not do
